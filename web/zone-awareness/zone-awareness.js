...
function renderLeaderboard(){
  ...
  $("#leaderboard").innerHTML=rows.map((p,i)=>`<tr data-id="${p.batter_id}" class="${p.batter_id===state.selected?'selected':''}"><td>${i+1}</td><td><strong>${p.batter_name}</strong><br><small>${p.team}</small></td><td class="${p.za_percentile>=50?'good':'bad'}">${fmt(p.za_percentile,1)}</td><td>${signed(p.za_raw,2)}</td><td>${signed(p.swing_aggression,2)}</td><td>${fmt(p.raw_dv,2)}</td><td>${fmt(p.dv_per_100,2)}</td><td>${p.pitches_seen.toLocaleString()}</td></tr>`).join("");
...
function drawScatter(){
  ...
  ctx.fillStyle="#758184";ctx.font="bold 12px Arial";ctx.fillText("Swing Aggression",w-128,h-13);ctx.save();ctx.translate(15,68);ctx.rotate(-Math.PI/2);ctx.fillText("ZA Raw",0,0);ctx.restore();
...
function bindScatter(){const canvas=$("#scatter"),tip=$("#scatter-tip");canvas.onmousemove=e=>{const q=pointer(canvas,e), hit=state.scatterPoints?.find(d=>Math.hypot(q.x-d.x,q.y-d.y)<d.r);if(!hit){tip.style.display="none";canvas.style.cursor="default";return;}canvas.style.cursor="pointer";tip.innerHTML=`<strong>${hit.p.batter_name}</strong> · ${hit.p.team}<br>ZA ${signed(hit.p.za_raw,2)} · ${fmt(hit.p.za_percentile,1)}%ile<br>SA ${signed(hit.p.swing_aggression,2)} · Raw DV ${fmt(hit.p.raw_dv,2)}`;tip.style.display="block";tip.style.left=`${Math.min(e.offsetX+12,canvas.clientWidth-170)}px`;tip.style.top=`${Math.max(4,e.offsetY-64)}px`;};canvas.onmouseleave=()=>tip.style.display="none";canvas.onclick=e=>{const q=pointer(canvas,e),hit=state.scatterPoints?.find(d=>Math.hypot(q.x-d.x,q.y-d.y)<d.r);if(hit)selectPlayer(hit.p.batter_id);};}
...
function renderPlayer(){const p=state.profile.summary;$("#player-name").textContent=p.batter_name;$("#player-meta").textContent=`${state.season} · ${p.team} · ${p.pitches_seen.toLocaleString()} PITCHES`;$("#score-sa").textContent=signed(p.swing_aggression,2);$("#score-za").textContent=signed(p.za_raw,2);$("#score-percentile").textContent=`${fmt(p.za_percentile,1)}%`;$("#score-dv").textContent=fmt(p.raw_dv,2);$("#score-dv100").textContent=fmt(p.dv_per_100,2);$("#score-za").className=signClass(p.za_raw);$("#score-percentile").className=p.za_percentile>=50?"good":"bad";$("#score-dv").className=signClass(p.raw_dv);$("#score-dv100").className=signClass(p.dv_per_100);drawRegionMap(p);renderActions(p);drawDecisionMap();renderOutcome();}

function regionData(p){return ["Heart","Shadow","Chase","Waste"].map(name=>{const k=name.toLowerCase();return{name,n:p[`${k}_pitches`],total:p[`${k}_decision_value_per_100`],swing:p[`${k}_swing_decision_value_per_100`],take:p[`${k}_take_decision_value_per_100`]};});}
function drawRegionMap(p){const {ctx,width:w,height:h}=canvasContext($("#region-map")),cx=w/2,cy=h/2,regions=[...regionData(p)].reverse(),sizes=[.84,.68,.52,.36];ctx.fillStyle="#f8f8f5";ctx.fillRect(0,0,w,h);const scale=Math.max(...regions.map(r=>Math.abs(r.total||0)),.25);regions.forEach((r,i)=>{const s=Math.min(w,h)*sizes[i];ctx.fillStyle=diverge(r.total,scale);ctx.fillRect(cx-s/2,cy-s/2,s,s);ctx.strokeStyle="#fff";ctx.lineWidth=2;ctx.strokeRect(cx-s/2,cy-s/2,s,s);ctx.fillStyle="#26343a";ctx.font="bold 12px Arial";ctx.fillText(`${r.name}  ${fmt(r.total,2)}`,cx-s/2+10,cy-s/2+20);});const zone=Math.min(w,h)*.36;ctx.strokeStyle="#176f84";ctx.lineWidth=2;ctx.strokeRect(cx-zone/2,cy-zone/2,zone,zone);ctx.lineWidth=1;for(let i=1;i<3;i++){ctx.beginPath();ctx.moveTo(cx-zone/2+i*zone/3,cy-zone/2);ctx.lineTo(cx-zone/2+i*zone/3,cy+zone/2);ctx.moveTo(cx-zone/2,cy-zone/2+i*zone/3);ctx.lineTo(cx+zone/2,cy-zone/2+i*zone/3);ctx.stroke();}}
...
function renderOutcome(){if(!state.profile)return;const c=state.cell||aggregateGrid();$("#cell-label").textContent=state.cell?`x ${fmt(c.x,2)} · z ${fmt(c.z,2)} · ${c.n}구`:"맵 전체 평균";$("#swing-path").innerHTML=pathRows([["Actual Swing",c.swing_pct],["Expected Swing",c.expected_swing_pct]]);$("#take-path").innerHTML=pathRows([["pZone",c.p_zone_pct],["Actual Judgment",c.zone_judgment_pct],["Expected Judgment",c.expected_zone_judgment_pct]]);$("#swing-rv").textContent=fmt(c.expected_swing_rv,3);$("#take-rv").textContent=fmt(c.expected_take_rv,3);const gap=c.expected_swing_rv-c.expected_take_rv;$("#rv-gap").textContent=`${gap>=0?'+':''}${fmt(gap,3)}`;$("#rv-gap").className=gap>=0?"good":"bad";}
