...
function renderLeaderboard(){
  ...
  $("#leaderboard").innerHTML=rows.map(p=>`<tr data-id="${p.batter_id}" class="${p.batter_id===state.selected?'selected':''}"><td>${p.rank}</td><td><strong>${p.batter_name}</strong><br><small>${p.team}</small></td><td>${signed(p.za_raw,2)}</td><td class="${p.za_percentile>=50?'good':'bad'}">${fmt(p.za_percentile,1)}</td><td>${signed(p.swing_aggression,2)}</td><td>${fmt(p.raw_dv,2)}</td><td>${signed(p.dv_per_100,2)}</td><td>${p.pitches_seen.toLocaleString()}</td></tr>`).join("");
