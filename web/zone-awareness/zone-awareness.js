...
async function loadSeason(season) {
  ...
  document.querySelector('th[data-sort="za_raw"]').textContent=state.modern?"ZA":"기존 ZA Raw";
...
function renderLeaderboard(){
  ...
  $("#leaderboard").innerHTML=rows.map(p=>`<tr data-id="${p.batter_id}" class="${p.batter_id===state.selected?'selected':''}"><td>${p.rank}</td><td><strong>${p.batter_name}</strong><br><small>${p.team}</small></td><td>${signed(p.za_raw,2)}</td><td class="${p.za_percentile>=50?'good':'bad'}">${fmt(p.za_percentile,1)}</td><td>${signed(p.swing_aggression,2)}</td><td>${fmt(p.raw_dv,2)}</td><td>${signed(p.dv_per_100,2)}</td><td>${fmt(p.dv_plus,1)}</td><td>${p.pitches_seen.toLocaleString()}</td></tr>`).join("");
...
function drawScatter(){
  ...
  ctx.fillStyle="#758184";ctx.font="bold 12px Arial";ctx.fillText("Swing Aggression",w-128,h-13);ctx.save();ctx.translate(15,68);ctx.rotate(-Math.PI/2);ctx.fillText(state.modern?"ZA":"기존 ZA Raw",0,0);ctx.restore();
