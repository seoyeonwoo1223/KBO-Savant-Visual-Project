...
function renderActions(p){const values=[{name:"Z attack (50%)",v:p.pure_z_attack_z_final},{name:"O restraint (50%)",v:p.pure_o_restraint_z_final},{name:"Heart–Chase residual",v:p.pure_hc_residual_z_adjusted},{name:"Z–O residual",v:p.pure_zo_residual_z_adjusted}],max=Math.max(.5,...values.map(x=>Math.abs(x.v||0)));$("#cluster-label").textContent=`Approach Cluster ${p.pure_cluster_id ?? '—'} · 조정 비중 50%`;$("#outcome-label").textContent=`진단 전용 · BB ${fmt(p.bb_pct,1)}% · K ${fmt(p.k_pct,1)}% · HBP ${fmt(p.hbp_pct,1)}%`;$("#action-bars").innerHTML=values.map(x=>{const width=Math.abs(x.v||0)/max*48,left=x.v>=0?50:50-width;return `<div class="zero-row"><span>${x.name}</span><div class="zero-track"><i class="zero-fill ${x.v<0?'negative':''}" style="left:${left}%;width:${width}%"></i></div><b class="${x.v>=0?'good':'bad'}">${fmt(x.v,2)}z</b></div>`}).join("");$("#region-table").innerHTML=regionData(p).map(r=>`<tr><td>${r.name}</td><td>${r.n.toLocaleString()}</td><td>${fmt(r.swing,1)}%</td><td>${fmt(r.correct,1)}%</td></tr>`).join("");}
