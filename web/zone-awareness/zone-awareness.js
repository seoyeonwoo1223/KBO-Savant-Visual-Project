...
const state = { season: null, players: [], sort: "seager_plus", direction: -1, selected: null, profile: null, cell: null };
...
async function loadSeason(season) {
  ...
  const player=state.players.find(p=>p.batter_id===requested) || [...state.players].sort((a,b)=>b.seager_plus-a.seager_plus)[0];
...
function renderLeaderboard(){
  ...
  rows.sort((a,b)=>{
    if(state.sort==="rank") return state.direction*(a.seager_plus-b.seager_plus);
  ...
  $("#leaderboard").innerHTML=rows.map((p,i)=>`<tr data-id="${p.batter_id}" class="${p.batter_id===state.selected?'selected':''}"><td>${i+1}</td><td><strong>${p.batter_name}</strong><br><small>${p.team}</small></td><td class="${plusClass(p.seager_plus)}">${fmt(p.seager_plus,0)}</td><td>${fmt(p.selection_tendency_pct,1)}</td><td>${fmt(p.hittable_pitches_taken_pct,1)}</td><td>${fmt(p.za_with_contact_plus,0)}</td><td>${p.pitches_seen.toLocaleString()}</td></tr>`).join("");
...
function drawScatter(){
  ...
  const xs=state.players.map(p=>100-p.hittable_pitches_taken_pct), ys=state.players.map(p=>p.selection_tendency_pct);
  const xLo=Math.floor((Math.min(...xs)-5)/10)*10, xHi=Math.ceil((Math.max(...xs)+5)/10)*10;
  const yLo=Math.floor((Math.min(...ys)-5)/10)*10, yHi=Math.ceil((Math.max(...ys)+5)/10)*10;
  ...
  for(let v=Math.ceil(xLo/10)*10;v<=xHi;v+=10){ctx.beginPath();ctx.moveTo(x(v),pad.t);ctx.lineTo(x(v),h-pad.b);ctx.stroke();ctx.fillText(v,x(v)-7,h-pad.b+18);}
  for(let v=Math.ceil(yLo/10)*10;v<=yHi;v+=10){ctx.beginPath();ctx.moveTo(pad.l,y(v));ctx.lineTo(w-pad.r,y(v));ctx.stroke();ctx.fillText(v,pad.l-28,y(v)+4);}
  ctx.fillStyle="#758184";ctx.font="bold 12px Arial";ctx.fillText("HPT Avoidance% →",w-132,h-13);ctx.save();ctx.translate(15,126);ctx.rotate(-Math.PI/2);ctx.fillText("Selection Tendency% →",0,0);ctx.restore();
  ctx.globalAlpha=.7;ctx.font="bold 11px Arial";ctx.fillText("SEAGER형 접근",w-pad.r-88,pad.t+16);ctx.globalAlpha=1;
  state.scatterPoints=[];
  state.players.forEach(p=>{const px=x(100-p.hittable_pitches_taken_pct),py=y(p.selection_tendency_pct),r=Math.max(3,Math.min(8,Math.sqrt(p.pitches_seen)/6));ctx.beginPath();ctx.arc(px,py,r,0,Math.PI*2);ctx.fillStyle=plusColor(p.seager_plus);ctx.fill();ctx.strokeStyle=p.batter_id===state.selected?"#17272c":"#ffffff";ctx.lineWidth=p.batter_id===state.selected?2.4:1;ctx.stroke();state.scatterPoints.push({p,x:px,y:py,r:r+4});});
  canvas._chart={w,h};
}
...
function bindScatter(){const canvas=$("#scatter"),tip=$("#scatter-tip");canvas.onmousemove=e=>{const q=pointer(canvas,e), hit=state.scatterPoints?.find(d=>Math.hypot(q.x-d.x,q.y-d.y)<d.r);if(!hit){tip.style.display="none";canvas.style.cursor="default";return;}canvas.style.cursor="pointer";tip.innerHTML=`<strong>${hit.p.batter_name}</strong> · ${hit.p.team}<br>SEAGER+ ${fmt(hit.p.seager_plus,0)} · Selection ${fmt(hit.p.selection_tendency_pct,1)}% · HPT ${fmt(hit.p.hittable_pitches_taken_pct,1)}%`;tip.style.display="block";tip.style.left=`${Math.min(e.offsetX+12,canvas.clientWidth-190)}px`;tip.style.top=`${Math.max(4,e.offsetY-48)}px`;};canvas.onmouseleave=()=>tip.style.display="none";canvas.onclick=e=>{const q=pointer(canvas,e),hit=state.scatterPoints?.find(d=>Math.hypot(q.x-d.x,q.y-d.y)<d.r);if(hit)selectPlayer(hit.p.batter_id);};}
...
function renderPlayer(){const p=state.profile.summary;$("#player-name").textContent=p.batter_name;$("#player-meta").textContent=`${state.season} · ${p.team} · ${p.pitches_seen.toLocaleString()} PITCHES`;[["#score-za",p.seager_plus,0],["#score-z",p.selection_tendency_pct,1],["#score-o",p.hittable_pitches_taken_pct,1],["#score-con",p.za_with_contact_plus,0]].forEach(([s,v,d],i)=>{$(s).textContent=fmt(v,d);$(s).className=i===0||i===3?plusClass(v):"";});drawRegionMap(p);renderActions(p);drawDecisionMap();renderOutcome();}
...
function drawRegionMap(p){const {ctx,width:w,height:h}=canvasContext($("#region-map")),cx=w/2,cy=h/2,regions=[...regionData(p)].reverse(),sizes=[.92,.736,.49,.368,.245];ctx.fillStyle="#f8f8f5";ctx.fillRect(0,0,w,h);regions.forEach((r,i)=>{const s=Math.min(w,h)*sizes[i];ctx.fillStyle=diverge((r.swing-50)/50,1);ctx.fillRect(cx-s/2,cy-s/2,s,s);ctx.strokeStyle="#fff";ctx.lineWidth=2;ctx.strokeRect(cx-s/2,cy-s/2,s,s);ctx.fillStyle="#26343a";ctx.font="bold 12px Arial";ctx.fillText(`${r.name}  Swing ${fmt(r.swing,0)}%`,cx-s/2+10,cy-s/2+20);});const zone=Math.min(w,h)*.368;ctx.strokeStyle="#176f84";ctx.lineWidth=2;ctx.strokeRect(cx-zone/2,cy-zone/2,zone,zone);ctx.lineWidth=1;for(let i=1;i<3;i++){ctx.beginPath();ctx.moveTo(cx-zone/2+i*zone/3,cy-zone/2);ctx.lineTo(cx-zone/2+i*zone/3,cy+zone/2);ctx.moveTo(cx-zone/2,cy-zone/2+i*zone/3);ctx.lineTo(cx+zone/2,cy-zone/2+i*zone/3);ctx.stroke();}}
function renderActions(p){const total=p.pitches_seen||1,values=[{name:"A · 칠 공 스윙",n:p.seager_a_hittable_swings,good:true},{name:"B · 피할 공 스윙",n:p.seager_b_avoid_swings,good:false},{name:"C · 칠 공 테이크",n:p.seager_c_hittable_takes,good:false},{name:"D · 피할 공 테이크",n:p.seager_d_avoid_takes,good:true}].map(x=>({...x,v:100*x.n/total}));$("#cluster-label").textContent=`SEAGER ${fmt(p.seager_raw,1)} · SEAGER+ ${fmt(p.seager_plus,0)}`;$("#outcome-label").textContent=`Selection ${fmt(p.selection_tendency_pct,1)}% − HPT ${fmt(p.hittable_pitches_taken_pct,1)}% · 올바른 선택 ${fmt(p.seager_correct_pct,1)}%`;$("#action-bars").innerHTML=values.map(x=>`<div class="zero-row"><span>${x.name}</span><div class="zero-track"><i class="zero-fill ${x.good?'':'negative'}" style="left:0;width:${x.v}%"></i></div><b class="${x.good?'good':'bad'}">${x.n.toLocaleString()} · ${fmt(x.v,1)}%</b></div>`).join("");$("#region-table").innerHTML=regionData(p).map(r=>`<tr><td>${r.name}</td><td>${r.n.toLocaleString()}</td><td>${fmt(r.swing,1)}%</td></tr>`).join("");}
