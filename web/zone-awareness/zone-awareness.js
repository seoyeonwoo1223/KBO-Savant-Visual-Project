...
function renderLeaderboard(){
  ...
  $("#leaderboard").innerHTML=rows.map((p,i)=>`<tr data-id="${p.batter_id}" class="${p.batter_id===state.selected?'selected':''}"><td>${i+1}</td><td><strong>${p.batter_name}</strong><br><small>${p.team}</small></td><td class="${plusClass(p.zone_awareness_plus)}">${fmt(p.zone_awareness_plus,0)}</td><td>${fmt(p.z_zone_awareness_plus,0)}</td><td>${fmt(p.o_zone_awareness_plus,0)}</td><td>${p.pitches_seen.toLocaleString()}</td></tr>`).join("");
...
function renderPlayer(){const p=state.profile.summary;$("#player-name").textContent=p.batter_name;$("#player-meta").textContent=`${state.season} · ${p.team} · ${p.pitches_seen.toLocaleString()} PITCHES`;[["#score-za",p.zone_awareness_plus],["#score-z",p.z_zone_awareness_plus],["#score-o",p.o_zone_awareness_plus]].forEach(([s,v])=>{$(s).textContent=fmt(v,0);$(s).className=plusClass(v);});drawRegionMap(p);renderActions(p);drawDecisionMap();renderOutcome();}
