...
fetch(`../data/profiles/${profile}.json`).then(response => response.json()).then(data => {
  ...
  document.querySelector("#overall").innerHTML = cell("Total Decision Run", data.overall.decision_run) + cell("per 100 pitches", data.overall.decision_run_per_100) + cell("Pitches", data.overall.pitches) + cell("Swing%", data.overall.swing_pct) + cell("Take%", data.overall.take_pct);
  document.querySelector("#regions").innerHTML = Object.entries(data.regions).map(([name, value]) => `<article><h3>${name}</h3><p>${value.pitches.toLocaleString()}구 · ${value.share_pct}%</p><div class="split"><span>Swing ${value.swing.pitches}구 · ${fmt(value.swing_pct)}% <b>${fmt(value.swing.decision_run)}</b></span><span>Take ${value.take.pitches}구 · ${fmt(value.take_pct)}% <b>${fmt(value.take.decision_run)}</b></span></div></article>`).join("");
