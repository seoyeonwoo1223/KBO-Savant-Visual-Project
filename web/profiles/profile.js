...
fetch(`../data/profiles/${profile}.json`).then(response => response.json()).then(data => {
  ...
  document.querySelector("#meta").textContent = `${data.season} 정규시즌 · ${sample.eligible_pitches.toLocaleString()}구 · ${data.generated_at.slice(0, 10)} 기준${sample.meets_minimum ? "" : " · 표본 100구 미만"}`;
