...
작업 결과 보고는 아래 세 절만 쓴다. 보고 전용 서브에이전트를 만들지 않는다 — 보고는 맥락을 아끼는 작업이 아니라 이미 가진 맥락을 쓰는 작업이다.

### Summary
무엇이 바뀌었는지 짧게.

### Validation
실제로 무엇을 실행·확인했는지. 돌리지 않은 것을 돌렸다고 쓰지 않는다.

### Issues
미해결 위험과 블로커만. 없으면 이 절을 생략한다.

작업 중에는 중요한 발견·블로커·방향 전환만 보고한다. 파일 탐색, 읽기, 편집, 테스트 실행은 중계하지 않는다. 과제를 다시 서술하거나, 만진 파일·명령을 나열하거나, 같은 결론을 반복하지 않는다.

서브에이전트를 호출할 때 **프롬프트는 자립형이어야 한다** — 저장소 경로, 기준 커밋, 무엇이 검증됐고 무엇이 미검증인지를 호출 시 명시한다. 서브에이전트는 이전 대화 맥락을 보지 못한다.

## 지표 개발 중 검증의 위치

SBJ·APR+·DV 같은 목표 지표를 개발할 때, ABS 존 경계값 재검증·오차 확인 같은 중간 검증에 연속으로 매달리지 않는다. 목표는 지표 완성이지 중간 검증 자체가 아니다. **검증은 한 번 통과하면 다음 검증으로 넘어가지 말고 바로 다음 목표 단계로 진행한다.** 통과한 검증을 다른 각도에서 다시 확인하려는 흐름이 보이면 그 자리에서 경고하고 넘어간다.
