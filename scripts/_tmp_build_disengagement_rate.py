...
def main() -> None:
    ...
    made_sorted = sorted(
        rows,
        key=lambda row: (-(row["disengagements_per_100_runner_pitches"] or -1), row["team"]),
    )
    ...
    assert total_made == total_received, (total_made, total_received)
