...
| 5차 개정 | 두 번째 외부 리뷰 반영: 2024년 공개 값 변경 범위 명시, 선수별 검정의 다중비교 보정, 정의 변환 주장 하향, 선수 ID 대응표 구축(8.1-5·8.6) |
...
2. **고치되 크게 투자하지 마십시오.** 같은 HGB·같은 날짜 블록 OOF로 비교하면 판정면 입력은 테이크 log loss를 0.0405→0.0277, 경계 구종별 보정 오차를 최대 0.316→0.013으로 줄입니다. 선수 SBJ 순위 상관은 시즌별 0.995–0.996이고, 변화 크기는 점수 자체의 표준오차(중앙값 약 1.1)보다 작습니다. 같은 경기로 묶은 **차이**의 표준오차(약 0.17)로 선수마다 검정하면 명목상 95% 기준을 넘는 선수가 시즌마다 8–15명이지만, 다중비교를 보정하면 0명입니다. 개별 선수의 변화가 확실하다고 말할 근거는 없습니다(7.4절).
...
| 시즌 | 적격 | Spearman | \|ΔSBJ\| 중앙값 / p90 / 최대 | (a) 점수 SE 중앙값, \|Δ\| > 점수 SE | (b) 차이 SE 중앙값, 명목 \|z\| > 1.96 (우연 기대) | (b) 보정 후: BH FDR 5% / Bonferroni | 최대 순위 이동 |
|---|---|---|---|---|---|---|---|
| 2024 | 155 | 0.9961 | 0.138 / 0.353 / 0.711 | 0.99, 0명 | 0.169, 15명 (7.8명) | 0명 / 0명 | 15 |
| 2025 | 171 | 0.9945 | 0.135 / 0.354 / 1.097 | 1.14, 0명 | 0.178, 8명 (8.6명) | 0명 / 0명 | 19 |
| 2026 | 166 | 0.9951 | 0.151 / 0.425 / 0.731 | 1.12, 0명 | 0.184, 14명 (8.3명) | 0명 / 0명 | 12 |

  - 두 표준오차는 모두 경기 단위 군집 표준오차입니다. (a)는 기존 점수의 표본 불확실성과 비교한 **실무적 크기**이고, (b)는 같은 투구·같은 경기에서 두 모델 점수의 **차이**를 직접 추정한 것입니다.
  - **(b)의 해석 (5차 개정에서 정정):** 명목 기준 초과 수는 선수마다 5% 검정을 반복한 **보정 전** 결과입니다.
    - 우연만으로도 시즌당 약 8명이 선을 넘습니다. 2025년 8명은 우연 수준이고, 2024·2026년은 기대의 약 2배입니다(선수 간 독립을 가정한 이항 꼬리확률 0.011 / 0.040). 그러나 선수들은 같은 모델을 공유하므로 독립 가정이 성립하지 않아 강한 증거가 아닙니다.
    - BH(FDR 5%)나 Bonferroni로 보정하면 세 시즌 모두 **0명**입니다.
    - 이 표준오차는 학습된 두 모델을 고정하고 투구 표본의 변동만 반영합니다. 모델을 다시 학습할 때의 변동은 들어 있지 않습니다.
    - 따라서 말할 수 있는 것은 "판정면 교체는 선수 점수를 대체로 작게 바꾸고, 개별 선수의 변화가 확실하다고 보기는 어렵다"까지입니다. 4차 개정의 "일부 선수에게는 체계적인 수정" 표현은 철회합니다. 1차 개정의 "표준오차 이내"는 (a)만 본 것이고, (a)는 실무적 크기의 지표이지 두 모델이 같다는 검정이 아닙니다.
...
- **2024년 공개 값 변경 범위 (5차 개정에서 명시):** DV·SA의 **산식은 이 PR에서 바꾸지 않았지만, 2024년의 공개 값은 재빌드 때문에 바뀝니다.** 화면에 표시되는 자릿수(`zone-awareness.js`: ZA·SA·DV·DV/100 소수 2자리, 백분위·DV+ 소수 1자리)로 반올림해 비교하면, 적격 155명 중 값이 달라지는 선수는 다음과 같습니다(외부 리뷰의 독립 집계와 같은 값).

  | 시즌 | ZA(SBJ) | 백분위 | SA | DV | DV/100 | DV+ |
  |---|---|---|---|---|---|---|
  | 2024 (적격 155) | 154 | 123 | **143** | **153** | **116** | **130** |
  | 2025 (적격 171) | 169 | 146 | 0 | 0 | 0 | 0 |

  - 크기는 작습니다(DV/100 변화 중앙값 0.013·최대 0.085, 순위 상관 0.9996, 투구 수 동일). 그러나 "작게 바뀐다"와 "바뀌지 않는다"는 다릅니다.
  - 원인에 대해서는 **제 재현 보고**로 구분해 적습니다. master 코드로 2024년을 다시 빌드해도 DV·SA가 똑같이 바뀌므로, 원래 커밋돼 있던 2024년 산출물이 현재 코드·데이터와 어긋나 있던 것으로 봅니다. PR 코드와 master 코드로 빌드한 결과의 차이는 `za_raw`·`za_percentile`뿐입니다. `p_zone`이 DV·SA 계산에 들어가지 않는 코드 구조와도 맞습니다. 외부 리뷰어는 이 재빌드를 다시 실행하지 않았습니다.
  - 재현성: 같은 환경에서 새로 빌드하면 커밋된 2024·2025년 산출물과 바이트 단위로 같습니다.
  - **2024년의 이 부수적 공개 값 갱신을 이 PR에 함께 받을지는 사용자 결정 사항입니다.**
...
3. **VB와 TrackMan의 무브먼트 스케일 차이(2024년 0.84–0.85)는 측정 구간 정의 차이와 방향·규모가 맞지만, 정의만으로 설명된다는 정량 결론은 입증되지 않았습니다 (5차 개정에서 하향).** VB는 50 ft부터, TrackMan은 릴리스부터 플레이트까지를 씁니다(TrackMan 공식 정의). 구간 거리비의 제곱 f = ((50 − 17/12) / (60.5 − extension − 17/12))²는 중앙값 0.824로 관측 스케일 0.847에 가깝습니다.
   - 그러나 TrackMan 값에 f를 곱한 뒤 기울기가 0.847 → 1.028이 되는 것은 **거의 계산상 자동으로 나오는 결과**입니다(0.847 / 0.824 ≈ 1.03). 이것만으로는 증거가 되지 않습니다.
   - 이와 별개인 두 검정은 약하거나 어긋납니다. ① 포심(IVB > 25cm)만 보면 투수별 VB/TrackMan 비율의 중앙값이 0.901로 예측 0.826과 9% 다릅니다. ② extension이 클수록 비율이 커져야 하는데(예측 기울기 +0.10 / m), 관측은 +0.16(상관 0.26)이지만 주 구장별로 중심화하면 +0.04(상관 0.11)로 약해집니다.
   - 투구 단위 잔차(MAD 3.8 / 5.0cm)는 이 변환으로 줄지 않습니다.
   - 결론: 스케일을 "오류"로 보고 VB를 TrackMan 원값에 맞춰 고치면 안 된다는 판단은 유지합니다. 다만 그 근거는 "정의가 다르다는 사실"이지 "정의로 다 설명된다는 정량 증명"이 아닙니다.
4. **기존 워크북의 2025년 HB 오프셋은 부호가 반대인 것으로 진단됩니다.** 서로 다른 두 추정 방식에서 같은 결론이 나왔지만, 워크북 작성 과정을 확인하기 전에는 원인(부호 규약 오류)까지 확정한 것은 아닙니다. 같은 투수·같은 구종 기준으로 잰 실측 구장 편향을 워크북 오프셋에 회귀했습니다. 보정이 제대로 되어 있다면 음의 상관이어야 합니다(오프셋 = −편향).
   ...
   - **2원 고정효과 재추정 (4차 개정):** 투수×구종 효과와 구장×구종 효과를 동시에 추정하는 방식(교대 평균 제거)으로 다시 쟀습니다. 구장 효과 범위는 HB 22–31cm(2023–25) / 12cm(2026), IVB 11–34cm로, 중앙값 방식보다 큽니다(위 주의와 일치). 워크북 오프셋과의 상관은 HB 2023 −0.23, 2024 −0.25, **2025 +0.91**, 2026 −0.16이고, IVB는 −0.62, −0.92(기울기 −0.97), −0.95, −0.95입니다. 2025년 HB의 양의 상관과 2024년 IVB의 정상 보정이 두 방식에서 모두 재현됩니다. 다만 두 방식은 같은 VB·워크북 자료를 쓰므로 **외부의 독립 확인은 아닙니다.** 원인 확정은 워크북 작성 과정을 확인해야 가능합니다.
5. **공통 투구 ID 프로토타입 (2019·2024년)**
   ...
   - **투수 불일치의 원인 (2024):** 불일치 행에 나오는 TrackMan 투수 ID는 73개(예: 658792)이고, 불일치 행의 99.6%에서 그 ID가 VB ID 체계에 아예 없습니다. 73개 중 85%는 특정 VB ID 하나와 일관되게 대응하므로 **선수 ID 대응표(crosswalk)가 필요합니다.** `docs/tracking-data.md`의 "같은 시즌 문자열 동등 조인"은 이 선수들을 조용히 빼 버립니다.
   - **5차 개정 — 확인과 대응표 구축:** 대응 쌍의 VB 이름으로 **외국인 선수임을 확인했습니다**(투수 윌커슨·엔스·헤이수스·하트·레예스·코너, 타자 로하스·레이예스·오스틴·데이비슨·페라자·에레디아 등). TrackMan이 외국인 선수에게 MLB식 ID를 씁니다.
   - `scripts/build_trackman_id_crosswalk.py`로 `data/tracking/player_id_crosswalk.json`을 만들었습니다(규칙은 `docs/tracking-data.md`). 2019–2024 KBO 경기 TrackMan 투구의 연결률은 투수 기준 문자열 일치 75.2–81.3%에서 대응표+문자열 97.9–98.6%로, 타자 기준 87.9–91.5%에서 98.9–99.4%로 오릅니다.
   - 다른 ID 쌍은 시즌마다 투수 16–31명, 타자 12–17명입니다. 문자열 일치와 대응표가 서로 다른 VB ID를 내는 경우는 0건이고, 한 TrackMan ID가 시즌마다 다른 VB ID로 연결된 경우도 0건입니다.
   - 거부 사례: 정렬 오류로 보이는 2건(역방향 점유율 0.03–0.05)과, 표본이 20구 미만인 실제 쌍(예: 2022년 이대호의 은퇴 경기 등판)입니다.
   - 이것은 선수 단위 산출물이고 값을 바꾸지 않습니다. 투구 단위 매핑은 공개하지 않았습니다.
...
- **지금 권할 최소안 (승인 시):**
  1. **선수 ID 대응표와 검증된 공통 투구 매핑**을 데이터 계약으로 만듭니다. "공통 투구 ID를 먼저 만들자"는 사용자 의견을 받아들이되, 목적은 보정 입력이 아니라 **검증 인프라**입니다. → 5차 개정에서 **선수 ID 대응표는 구축했습니다**(8.1-5). 투구 단위 매핑의 공개는 아직 하지 않았습니다.
...
`isolate.py` — 7.4절 선수 SBJ 변화, 두 표준오차, 다중비교 보정. 해당 시즌 ZA 빌드 후 실행합니다.
...
import sys, pathlib; sys.path.insert(0, 'src')
import numpy as np, pandas as pd
from scipy.stats import spearmanr, norm, binom
...
for yr in map(int, sys.argv[1:]):
    ...
    n = len(r); nominal = int((z.abs() > 1.96).sum()); pv = 2 * norm.sf(z.abs().to_numpy())
    order = np.argsort(pv); bh = pv[order] * n / np.arange(1, n + 1); bh = np.minimum.accumulate(bh[::-1])[::-1]
    print(f"{yr}: 적격 {n} | Spearman {spearmanr(r.old, r.new)[0]:.4f} | |ΔSBJ| 중앙값 {dl.median():.3f} p90 {dl.quantile(.9):.3f} 최대 {dl.max():.3f} | "
          f"(a) 점수 SE 중앙값 {r.se_score.median():.2f}, |Δ|>점수 SE {int((dl > r.se_score).sum())}명 | "
          f"(b) 차이 SE 중앙값 {r.se_diff.median():.3f}, 명목 |z|>1.96 {nominal}명 (우연 기대 {0.05*n:.1f}명, 독립 가정 시 P(X≥{nominal})={binom.sf(nominal-1, n, .05):.3f}), "
          f"BH FDR 5% {int((bh <= .05).sum())}명, Bonferroni {int((pv < .05/n).sum())}명 | 최대 순위 이동 {rk.max():.0f}")
```
...
`defconv.py` — 8.1-3 측정 구간 정의 변환(기계적 기울기 변화). 부록 A `trackman()`의 매칭 결과 `m`(TrackMan `extension` 포함)을 parquet으로 저장하고 그 경로를 인자로 줍니다.
...
`ext_ratio.py` — 8.1-3 정의 변환의 비기계적 검정. 입력은 `defconv.py`와 같습니다.

```python
"""정의 변환의 비기계적 검정: 투수별 VB/TrackMan IVB 비율이 extension에 따라 기하 예측대로 변하는가 (탐색 매칭 결과 parquet)."""
import sys
import numpy as np, pandas as pd
m = pd.read_parquet(sys.argv[1]); m['ext'] = pd.to_numeric(m.extension, errors='coerce')
f = m[(m.pitch_type_group == 'fastball') & (m.induced_vert_break > 25) & m.ext.between(1.0, 2.6)].copy()
f['ratio'] = f.vertical_movement_cm / f.induced_vert_break
g = f.groupby('pitcher_id').agg(n=('ratio', 'size'), ratio=('ratio', 'median'), ext=('ext', 'median'), park=('stadium', lambda s: s.mode().iat[0]))
g = g[g.n >= 100]
pred = lambda e: ((50 - 17/12) / (60.5 - 3.28084 * e - 17/12)) ** 2
s, _ = np.polyfit(g.ext, g.ratio, 1); ps, _ = np.polyfit(g.ext, pred(g.ext), 1)
print(f'투수 {len(g)}명: 비율 중앙값 {g.ratio.median():.3f}, 예측 {pred(g.ext).median():.3f}; extension 1m당 관측 {s:+.3f} (상관 {np.corrcoef(g.ext, g.ratio)[0,1]:+.2f}) vs 예측 {ps:+.3f}')
g['ratio_c'] = g.ratio - g.groupby('park').ratio.transform('median'); g['ext_c'] = g.ext - g.groupby('park').ext.transform('median')
s2, _ = np.polyfit(g.ext_c, g.ratio_c, 1); print(f'주 구장별 중심화 후: 관측 {s2:+.3f} (상관 {np.corrcoef(g.ext_c, g.ratio_c)[0,1]:+.2f})')
```

`pitch_id_proto.py` — 8.1-5 공통 투구 ID 프로토타입. 인자로 시즌을 줍니다. 선수 ID 대응표 자체는 `scripts/build_trackman_id_crosswalk.py`입니다.
