...
| 6차 개정 | 세 번째 외부 리뷰 반영: 대응표 부분 빌드의 시즌 삭제 결함 수정, "모두 외국인" 정정, 선수 연결률과 투구 검증 쌍 구분(8.1-5) |
| 7차 개정 | 네 번째 외부 리뷰 반영: "투구 검증 쌍"을 "학습 투구 내 ID 일치"(`in_sample_id_consistent_pitches`)로 개명, VB 입력 해시에 선수 이름 포함 |
...
- **배포 경로 (7차 개정에서 교체):** `daily_update`는 `--season` 없이 CLI를 불러 2026년만 다시 빌드합니다. 4차 개정에서는 2024·2025년 산출물을 PR에 직접 넣었지만, 그러면 병합 push가 `web/**`를 바꿔 `deploy-pages`가 즉시 실행되고, 2024·2025년 `za7.1`과 2026년 `za7`이 섞여 공개됩니다. 그래서 방식을 바꿨습니다.
  - PR에서 2024·2025년 웹 산출물과 CSV를 master 상태로 되돌렸습니다(`web/**` 변경 0개). 병합 push는 `src/**` 변경으로 `daily_update`만 실행합니다.
  - `daily_update`가 모든 실행에서 2024·2025년 `zone_decision`을 `--only zone_decision`으로 확인하고, 입력·코드 hash가 바뀐 경우에만 다시 계산합니다. 상태 파일이 없는 첫 실행에서는 한 번 빌드하고 상태를 만듭니다.
  - Release 업로드와 커밋보다 앞에서 `scripts/check_zone_decision_outputs.py`가 세 시즌의 모델 버전·선수 파일·CSV·상태 해시가 모두 맞는지 확인합니다. 실패하면 어떤 공개 산출물도 교체되지 않습니다. 통과하면 한 커밋으로 올라가고, `deploy-pages`의 `workflow_run`(성공 시)으로 한 번 배포됩니다.
  - `cancel-in-progress`를 `false`로 바꿔, 코드 push가 만든 실행이 예약 실행에 취소되지 않고 순서대로 돌게 했습니다.
- **2024년 공개 값 변경 범위 (5차 개정에서 명시):** DV·SA의 **산식은 이 PR에서 바꾸지 않았지만, 2024년의 공개 값은 재빌드 때문에 바뀝니다.** 화면에 표시되는 자릿수(`zone-awareness.js`: ZA·SA·DV·DV/100 소수 2자리, 백분위·DV+ 소수 1자리)로 반올림해 비교하면, 적격 155명 중 값이 달라지는 선수는 다음과 같습니다(외부 리뷰의 독립 집계와 같은 값).
  ...
  - 재현성: 같은 환경에서 두 번 빌드하면 2024·2025년 산출물이 바이트 단위로 같았습니다(4차 개정 시점 로컬 확인). 7차 개정부터는 이 산출물을 CI가 만듭니다.
  - **2024년 공개 값 갱신은 사용자가 승인했습니다(7차 개정 시점).** 산식 변경이 아니라 기존 저장 결과와 현재 코드의 불일치를 바로잡는 일회성 갱신입니다.
...
5. **공통 투구 ID 프로토타입 (2019·2024년)**
   ...
   - **5차 개정 — 대응표 구축 (6차 개정에서 정정):** 다른 ID로 연결된 148명은 모두 TrackMan ID가 6자리(MLB식)입니다. 대부분 외국인 선수이지만 **모두 외국인은 아닙니다.** 류현진·추신수·이대호·이대은·권광민·남태혁·문찬종·정수민·진우영 9명은 한국 선수입니다. 공통점은 미국 프로야구 경력으로 보이나(이름 기반 판단), 전수 확인은 하지 않았습니다. 5차 개정의 "외국인 선수임을 확인"은 틀린 일반화였습니다.
   - `scripts/build_trackman_id_crosswalk.py`로 `data/tracking/player_id_crosswalk.json`을 만들었습니다(규칙은 `docs/tracking-data.md`). 아래 수치는 종류가 다르니 섞지 마십시오.
     - **선수 연결률:** 2019–2024 KBO 경기의 모든 TrackMan 투구 중 선수 ID가 VB 선수로 연결되는 비율입니다. 투수는 문자열 일치 75.2–81.3%에서 대응표+문자열 97.9–98.6%로, 타자는 87.9–91.5%에서 98.9–99.4%로 오릅니다. 투구끼리 짝지어졌다는 뜻이 **아닙니다.**
     - **학습 투구 내 ID 일치 (7차 개정에서 개명, 이전 이름 "투구 검증 쌍"):** 경기·순서·카운트·아웃과 조회 후 투수·타자 ID가 모두 맞는 투구입니다. VB 전체 투구의 83.0–85.3%이고, 2024년은 185,896구(83.3%)입니다. 대응표를 학습한 바로 그 투구에서 센 내부 일치율이고, 8.3절의 "검증된 매핑" 조건 중 구속 차 검사가 빠져 있으므로 **최종 투구 매핑률이 아닙니다.** 투구 단위 매핑은 아직 저장하지 않았으므로 "투구별 매칭 미완료" 상태는 그대로입니다.
     - **충돌 0건의 뜻:** 문자열 일치와 대응표가 서로 다른 VB ID를 가리킨 선수가 0명이고, 한 TrackMan ID가 시즌마다 다른 VB ID로 연결된 경우도 0건입니다. 투구 단위의 불일치가 없다는 뜻이 아닙니다. 카운트·아웃이 맞은 투구 중 조회 후에도 ID가 다른 투구가 시즌당 투수 130–185구, 타자 178–309구 남습니다(2024년 169 / 229구).
     - 외부 리뷰어가 검증된 경기를 절반씩 나눠 한쪽으로 대응표를 만들고 다른 쪽에서 확인했을 때, ID 일치율은 투수 99.89%, 타자 99.86%였습니다. 대응표의 유용성은 강하게 뒷받침되지만, 매핑되지 않은 경기나 전체 투구 매핑의 정확도를 증명하지는 않습니다.
     - 다른 ID 쌍은 시즌마다 투수 16–31명, 타자 12–17명입니다. 대응표를 읽는 코드는 아직 없습니다(3번 미승인).
     - 6차 개정에서 고친 결함: `--seasons`로 일부 시즌만 빌드하면 파일이 그 시즌만으로 덮어써져 다른 시즌이 사라졌습니다. 옵션을 없애 매번 모든 시즌을 다시 만들게 했고(45초), 이를 막는 회귀 테스트는 예전 스크립트에서 실패함을 확인했습니다. 시즌마다 VB 입력 해시(`visualbaseball_sha256`)도 기록합니다.
