...
- **투구 검증 쌍** (`verified_pitch_pairs`): 경기·순서·카운트·아웃이 맞고, 조회 후 투수와 타자 ID까지 모두 맞는 투구다. VB 전체 투구의 83.0–85.3%이다(2024년 185,896구, 83.3%). 경기 매핑이 되지 않은 TrackMan 경기(시즌당 약 60–95경기)와 TrackMan이 없는 구장의 경기는 여기서 빠진다.
...
대응표는 선수 단위 산출물이다. 투구 단위 매핑은 저장하지 않으며, 현재 `src/`·`scripts/`에서 대응표를 읽는 소비자는 없다. 아래의 투구 단위 결합 제한은 그대로다. 빌드는 `summary.json`의 모든 시즌을 매번 다시 만들고(시즌 일부만 다시 쓰는 옵션은 없다), 시즌마다 TrackMan 파일 해시와 스크립트가 읽은 VB 값의 해시(`visualbaseball_sha256`)를 기록한다.
