...
`recent` checks new/failed/incomplete work and completed games in the latest seven days. `sample` deterministically selects eight completed games older than 30 days across the season. A sample y0/schema change or analysis-hash changes in at least two games warns, or expands to full reconcile with `--auto-reconcile`. Re-running after interruption is safe: each shard and manifest is replaced atomically, missing shards rebuild, and unchanged hashes are no-ops.
