...
def parse_game(payload: dict[str, Any], schedule_game: dict[str, Any] | None = None, season: int = 2026, naver_enrichment: NaverEnrichment | None = None) -> tuple[dict, list[dict], list[dict], int]:
    ...
    game = {"season": season, "game_date": f"{game_id[:4]}-{game_id[4:6]}-{game_id[6:8]}", "game_id": game_id,
            ...
            "away_starter_name": str(away.get("starter") or ""), "home_starter_name": str(home.get("starter") or ""),
            ...
            "source_url": f"https://visualbaseball.com/api/game/pbp?id={game_id}", "source_hash": sha256(json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode()).hexdigest(), "validation_status": "PENDING"}
...
def _pitch(game, event_seq, pa_id, number, game_number, pa, pitch, before, after, runs, terminal, status, batter_team, catcher, naver_enrichment, naver_occurrences):
    ...
    no_events = bool(naver_enrichment and naver_enrichment.coverage == "record_no_event")
    row = {"season": game["season"], "game_date": game["game_date"], "game_id": game["game_id"], "stadium": game["stadium"], "event_seq": event_seq, "pa_id": pa_id, "pitch_id": f"{game['game_id']}-{pa_id}-{number:02d}", "pitch_number": number, "game_pitch_number": game_number, "inning": before["inning"], "inning_half": before["inning_half"], "batter_id": str(pa.get("batterId", "")), "batter_name": pa.get("batter", ""), "batter_team": batter_team, "batter_position": str(pa.get("pos") or ""), "pa_rbi": int(pa.get("rbi") or 0), "batter_stance": pitch.get("stance"), "pitcher_id": str(pa.get("pitcherId", "")), "pitcher_name": pa.get("pitcher", ""), "catcher_id": catcher.get("id") or None, "catcher_name": catcher.get("name") or None, "catcher_source": catcher.get("source") or "unavailable", "pitch_type": PITCH_TYPE_NAMES.get(pitch_type_code, stuff), "pitch_type_code": pitch_type_code, "pitch_type_kr": stuff, "velocity_kmh": pitch.get("spd"), "velocity_mph": round(float(pitch.get("spd", 0)) * .621371, 1), "px": pitch.get("px"), "pz": pitch.get("pz"), "pitch_call_code": code, "pitch_result": _description(code, str(pa.get("result", ""))), "pa_result": pa.get("result", ""), "pa_type": pa.get("type", ""), "description": _description(code, str(pa.get("result", ""))), "is_swing": code in {"S", "F", "X"}, "is_take": code in {"B", "T"} or (terminal and str(pa.get("type", "")).lower() == "hbp"), "is_contact": code in {"F", "X"}, "is_in_play": code == "X", "is_pa_terminal": terminal, "runs_on_pitch": runs, "is_wild_pitch": naver_event.get("is_wild_pitch") if naver_event else (False if no_events else None), "is_passed_ball": naver_event.get("is_passed_ball") if naver_event else (False if no_events else None), "naver_pitch_id": naver_event.get("naver_pitch_id") if naver_event else None, "naver_match_status": "matched" if naver_event else ("record_no_event" if no_events else ("unavailable" if not naver_enrichment else "unmatched")), "parse_status": status, "fetched_at": game["fetched_at"], "source_url": game["source_url"]}
