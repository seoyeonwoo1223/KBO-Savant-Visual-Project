...
def _pitch(game, event_seq, pa_id, number, game_number, pa, pitch, before, after, runs, terminal, status):
    ...
    row = {"season": game["season"], "game_date": game["game_date"], "game_id": game["game_id"], "event_seq": event_seq, "pa_id": pa_id, "pitch_id": f"{game['game_id']}-{pa_id}-{number:02d}", "pitch_number": number, "game_pitch_number": game_number, "inning": before["inning"], "inning_half": before["inning_half"], "batter_id": str(pa.get("batterId", "")), "batter_name": pa.get("batter", ""), "pitcher_id": str(pa.get("pitcherId", "")), "pitcher_name": pa.get("pitcher", ""), "pitch_type": PITCH_TYPE_NAMES.get(pitch_type_code, stuff), "pitch_type_code": pitch_type_code, "pitch_type_kr": stuff, "velocity_kmh": pitch.get("spd"), "velocity_mph": round(float(pitch.get("spd", 0)) * .621371, 1), "px": pitch.get("px"), "pz": pitch.get("pz"), "pitch_call_code": code, "pitch_result": _description(code, str(pa.get("result", ""))), "pa_result": pa.get("result", ""), "description": _description(code, str(pa.get("result", ""))), "is_swing": code in {"S", "F", "X"}, "is_take": code in {"B", "T"}, "is_contact": code in {"F", "X"}, "is_in_play": code == "X", "is_pa_terminal": terminal, "runs_on_pitch": runs, "parse_status": status, "fetched_at": game["fetched_at"], "source_url": game["source_url"]}
