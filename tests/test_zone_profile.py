...
def test_zone_profile_builds_search_index_and_pitcher_payload():
    with tempfile.TemporaryDirectory() as directory:
        ...
        columns = [
            ...
            "is_swing", "is_contact", "is_in_play", "velocity_kmh", "is_pa_terminal", "pa_result",
        ]
        rows = [
            [2026, "ok", 99, "테스트투수", 88, "테스트타자", "직구", 0.1, 2.5, 3.5, 1.5, 0, 0, True, False, False, 150.0, False, ""],
            [2026, "ok", 99, "테스트투수", 88, "테스트타자", "직구", 0.2, 2.6, 3.5, 1.5, 0, 0, True, True, True, 148.0, True, "중안"],
        ]
        ...
        assert sum(record[5] for record in payload["records"]) == 2
        assert sum(record[7] for record in payload["records"]) == 1
        assert sum(record[14] for record in payload["records"]) == 1
